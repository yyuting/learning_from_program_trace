...
def main():
    dir = '/shaderml/playground/1x_1sample_venice'
    camdir = '/shaderml/playground/datas_venice'
    ...
    if False:
        ...
        render_single(os.path.join(dir, mode), 'render_venice_simplified_proxy_5_100', 'none', 'none', sys.argv[1:], nframes=nframes, log_intermediates=False, render_size = (640, 960), render_kw={'compute_g': False, 'compute_f': False, 'ground_truth_samples': 1, 'random_camera': True, 'camera_pos': camera_pos, 'is_tf': True, 'zero_samples': False, 'gname':'%s_small' % mode, 'collect_loop_and_features': True, 'last_only': True, 'texture_maps': texture_maps, 'use_texture_maps': True, 'render_t': render_t})
    ...
    if False:
        for mode in ['test_close', 'test_far', 'test_middle']:
            ...
            camera_pos_xyz = path_np(ro_seed)
            if mode == 'test_close':
                camera_pos_xyz[1] -= np.random.rand(nframes) * 0.5
            elif mode == 'test_far':
                camera_pos_xyz[1] += np.random.rand(nframes) * 0.5
            camera_pos_ang1 = -np.random.rand(nframes) * 0.2
            camera_pos_ang2 = np.random.rand(nframes) - 0.5
